"""Config flow for start2smart integration."""
import logging
...
@callback
def smartbridge_entries(hass: HomeAssistant):
...
class Start2SmartConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _host_in_configuration_exists(self, host) -> bool:
        ...
        if host in smartbridge_entries(self.hass):
            return True
        return False
    ...
    async def _test_connection(self, host):
        ...
        self._request = requests.Request("GET", host).prepare()

        try:
            with requests.Session() as sess:

                await self.hass.async_add_executor_job(sess.send, self._request)
                return True

        except (OSError, HTTPError, Timeout):
            self._errors[CONF_HOST] = "cannot_connect"
            _LOGGER.error(
                "Could not connect to Start2Smart Bridge at %s, check host ip address",
                host,
            )
        return False
